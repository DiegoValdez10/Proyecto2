...
from mathlib import normVector, dot, menosvector, RestaVector, reflectVector, addVectors, multVectorScalar, refractVector, totalInternalReflection, fresnel, magnitudvector
...
class Raytracer(object):
    def __init__(self, screen):
        ...
        self.envMap = None
    def setCamera(self, position, direction, up):
        self.camPosition = position
        self.camDirection = direction
        self.camUp = up
    ...
    def rtRayColor(self, intercept, rayDirection, recursion=0):
        ...
        if material.matType == OPAQUE:
            for light in self.lights:
                if light.lightType == "Ambient":
                    ...
                    if light.lightType == "Directional":
                        lightDir = menosvector(light.direction)
                    elif light.lightType == "Point":
                        lightDir = RestaVector(light.point, intercept.point)
                        lightDistance = magnitudvector(lightDir)
                        lightDir = normVector(lightDir)

                    shadowIntersect = self.rtCastRay(intercept.point, lightDir, intercept.obj)

                    if shadowIntersect == None or (
                            light.lightType == "Point" and shadowIntersect != None and shadowIntersect.distance > lightDistance):
            ...
            reflect = reflectVector(menosvector(rayDirection), intercept.normal)
            ...
            reflect = reflectVector(menosvector(rayDirection), intercept.normal)
            reflectOrigin = addVectors(reflect, bias) if outside else RestaVector(reflect, bias)
            ...
            if not totalInternalReflection(intercept.normal, rayDirection, 1.0, material.ior):
                ...
                refractOrigin = RestaVector(intercept.point, bias) if outside else addVectors(intercept.point, bias)
        ...
        lightColor = [(ambientColor[i] + diffuseColor[i] + specularColor[i] + reflectColor[i] + refractColor[i]) for i
                      in range(3)]
