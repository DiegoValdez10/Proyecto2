...
from mathlib import RestaVector, vectorMagnitude, normVector, dot, multVectorScalar, cross, multVectors, addVectors,baricentric
...
class Sphere(Shape):
    ...
    def ray_intersect(self, orig, dir):
        L = RestaVector(self.position, orig)
        ...
        normal = RestaVector(point, self.position)
...
class Plane(Shape):
    ...
    def ray_intersect(self, orig, dir):
        ...
        num = dot((RestaVector(self.position, orig)), self.normal)
        t = num / denom
...
class Disk(Plane):
    ...
    def ray_intersect(self, orig, dir):
        ...
        contactDistance = RestaVector(planeIntersect.point, self.position)
...
class Triangulo(Shape):
    ...
    def ray_intersect(self, orig, dir):
        vert0 = RestaVector(self.v1, self.v0)
        vert1 = RestaVector(self.v2, self.v1)
        vert2 = RestaVector(self.v0, self.v2)
        normal = cross(vert0, RestaVector(self.v2, self.v0))
        normal = normVector(normal)
        ...
        vp0 = RestaVector(p, self.v0)
        vp1 = RestaVector(p, self.v1)
        vp2 = RestaVector(p, self.v2)
        ...
        uv = baricentric(self.v0, self.v1, self.v2, p)
...
class Cilindro(Shape):
    ...
    def ray_intersect(self, orig, dir):
        a = dir[0] ** 2 + dir[2] ** 2
        oc = RestaVector(orig, self.position)
